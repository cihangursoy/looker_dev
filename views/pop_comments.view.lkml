include: "/views/pop_support.lkml" #include the helper fields that are core to the PoP implementation - meaning: include the file in which you pasted the code as described in Step 1 above.
include: "/views/comments.view" #!Include the file that defines your base view here so you can refine it



view: +comments {#!Update to point to your view name (with the '+' making it a refinement).  That view's file must be included here, and then THIS file must be included in the explore
  ...
  dimension_group: time_ts {
    ...
    dimension: selected_period_size {
      ...
      sql:{%if pop_support.period_size._parameter_value != 'Default'%}{{pop_support.period_size._parameter_value}}
        ...
        time_ts_date._is_selected %}Day

                  {% elsif

        time_ts_month._is_selected %}Month
      ...
      dimension: time_ts_date_periods_ago_pivot {#!Update to match your base field name. This is generic sql logic (so you might expect it to be in pop_support template), but it is helpful to manifest this lynchpin pivot field here so we can create a dedicated pivot field in this specific date dimension's group label.

        label: "{% if _field._in_query%}Pop Period (Time Ts {{selected_period_size._sql}}){%else%} Pivot for Period Over Period{%endif%}"#makes the 'PIVOT ME' instruction clear in the field picker, but uses a dynamic output label based on the period size selected

        group_label: "Time Ts Date" #!Update this group label if necessary to make it fall in your date field's group_label
